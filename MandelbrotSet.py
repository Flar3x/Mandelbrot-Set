...
def Mandelbrot(meshg_a, meshg_b, J):
    '''
    ...
    for i in range(len(meshg_a)):
        ...
        for j in range(len(meshg_a[0])):
            ...
            #print(meshg_a[i][j], meshg_b[i][j], proxy)
            temp[0][j] = proxy
        output[i] = temp
    return output

zz = Mandelbrot(xx, yy, 100)
...
ax.contourf(xx, yy, zz, 10, cmap='bone')
plt.axis('off')
plt.savefig('Mandelbrot_BW_NoAxes.png', dpi = 300)
# # plt.fill(x,y)
